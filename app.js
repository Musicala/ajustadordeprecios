...
function handleAction(action) {
  ...
  if (action.type === "static_table") {
    return openStaticTableModal(action.title || "Datos", action.columns || [], action.rows || []);
  }
...
async function openTableModal(title, src, columnsWanted) {
  ...
  if (dlBtn) { dlBtn.style.display = ""; dlBtn.setAttribute("href", src); }
  try {
...
/* -------- Static table (demo: precios, contactos, etc.) -------- */
function openStaticTableModal(title, columns, rows) {
  $("#tableModalTitle").textContent = title || "Datos";
  ...
  const headers = (Array.isArray(columns) && columns.length) ? columns : ["Campo", "Valor"];
  const data = (Array.isArray(rows) ? rows : []).map(r => Array.isArray(r) ? r : [String(r)]);
